...
def validate_codex_package(session_dir: Path, workspace: Path | str | None = None, *, stale_running_seconds: int = 3600) -> dict[str, Any]:
    ...
    if status == "RUNNING":
        ...
        if started_at:
            try:
                ...
                stale_running = (datetime.now(timezone.utc) - started).total_seconds() >= stale_running_seconds
            except ValueError:
                stale_running = True
    if stale_running or (status == "RUNNING" and missing_files):
    ...
    if lifecycle_state == "VALIDATED_RAW":
        ...
        q_report = {
            ...
            "raw_package_path": rel(session_dir, root),
            "validation_report_path": rel(report_path, root),
            "reason": "stale or incomplete RUNNING capture" if lifecycle_state == "QUARANTINED_INCOMPLETE" else "manifest schema failure",
            "missing_files": missing_files,
...
def replay_traceability_ledger(workspace: Path | str | None = None, *, session_id: str | None = None) -> dict[str, Any]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if session_id and row.get("source_session_id") != session_id:
            continue
        artifact_keys = [key for key, value in row.items() if key.endswith("_path") and value]
        ...
        chains.append({"source_session_id": row.get("source_session_id"), "lesson_id": row.get("lesson_id"), "ok": bool(ok), "missing": missing})
...
def run_e2e_traceability_for_session(session_id: str, workspace: Path | str | None = None) -> dict[str, Any]:
    ...
    if validation["lifecycle_state"] != "VALIDATED_RAW":
        return {"status": "FAILED_VALIDATION", "validation": validation}
