...
# A hand-authored JSON matching the schema is indistinguishable from a real
# Codex CLI run unless the evidence is anchored to an actual transcript file.
# This floor is not a content check — it only rules out an empty/placeholder
# file; a genuine Codex CLI session transcript is reliably far larger.
...
def _codex_audit_probe(root: Path, path: Path | None) -> dict[str, Any]:
    ...
    if not isinstance(raw_transcript, dict):
        reasons.append("raw Codex CLI transcript evidence is missing")
    else:
